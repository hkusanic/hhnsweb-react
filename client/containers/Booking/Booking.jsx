	...
	componentWillReceiveProps(nextProps) {
		...
		if (this.props !== nextProps) {
			...
			if (this.props.appointment.appointmentData.Appointment) {
				this.handleDarshanApproved(
					this.props.appointment.appointmentData.Appointment
				);
			}
		}
			...
			return (
				<div>
					...
					<div>
						Your request for{' '}
						<span style={{ fontWeight: '400' }}>
							{this.state.DarshanRequested} minutes
						</span>{' '}
						with His Holiness Niranjana Swami is under review.&nbsp;
					</div>
					<div>
						Once it is approved you will receive an email with further
						instructions.
					</div>
					<div>&nbsp;</div>
			...
			return (
				<div>
					...
					<div>
						Ваш запрос на{' '}
						<span style={{ fontWeight: '400' }}>
							{this.state.DarshanRequested} Даршан
						</span>{' '}
						с Его Святейшеством Ниранджаной Свами находится на
						рассмотрении.&nbsp;
					</div>
					<div>
						Как только он будет одобрен, вы получите письмо с дальнейшими
						инструкциями.
					</div>
					<div>&nbsp;</div>
					<div>Ваши слуги,</div>
		...
		if (reactCookie.load('languageCode') === 'en') {
			return (
				<div>
					...
					<p>
						We see that you have a confirmed request for Darshan with H.H.
						Niranjana Swami.
					</p>
			...
			return (
				<div>
					...
					<p>
						Мы видим, что у вас есть подтвержденный запрос на даршан с Его
						Святейшеством Ниранджаной Свами.
					</p>
		...
		return (
			<div>
				<section
					...
					style={{
						backgroundImage:
							'url(https://ik.imagekit.io/gcwjdmqwwznjl/Booking_v1_BJC5u2LU4.png)',
					}}
				>
					<div class="breadcrumbs-custom-inner headingImage">
						<div class="container breadcrumbs-custom-container">
							<ul class="breadcrumbs-custom-path">
								...
								<li>
									<a className="textColor">Booking</a>
								</li>
							</ul>
						</div>
					</div>
				...
				{!this.state.isUserLogin ? (
					this.props.appointment.isSubmitted ? (
						<div className="requestDiv">
							<p className="Bookingsubmit">{this.ApprovedText()}</p>
						</div>
					) : !this.props.appointment.isSubmitted &&
					  !this.props.appointment.appointmentData &&
					  !this.props.appointment.appointmentData.Appointment ? (
						<div>
					  ...
					  this.props.appointment.appointmentData.Appointment &&
					  !this.props.appointment.appointmentData.Appointment.approved &&
